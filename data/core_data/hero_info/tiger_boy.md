# Hero Summary
Utility support providing stuns and card manipulation
...
## Hero Description
Tiger Boy is a utility support who exists to provide stuns in fights. That is essentially his only purpose, but it's a fairly useful one - after all, a stunned enemy is one who isn't attacking you back.
...
### Card Analysis
WAY too expensive for what it is. Yes, it can set you up for an isolated fight in the right circumstances - but 2 SP for that effect, if it hits, and if you have relevant enemies inside the small 1 hex range, is far too much. You'll almost never want to play this. Renamed to Prank Strike in v3.1, presumably to indicate that its existence is a prank on the player.
...
### Gained
- Tiger Boy's stun effect is only applied on odd numbered rounds.

### Removed
- Nothing

### Changed
- Tiger Boy's stun trigger changed from being limited to one activation per target to being able to activate repeatedly.
- Stunned targets now take 30% more damage for one round.
...
[N.B. It's entirely possible this means odd numbered fight rounds rather than game rounds, but the game steadfastly refuses to give me this Variant for me to check it. And honestly, I'm not sure I care about Tiger Boy enough to worry about that fact. If it does, that just means a direct nerf if you have the re-stun mastery or for shorter fights.]
...
### Changed
- Tiger Boy's stun trigger changed from applying once to each enemy unit to applying whenever he attacks, up to a number of attacks equal to the number of enemy units.
...
# Item Build
Support-Tank. Again, his effect is agnostic of stats, so grab the support items you want then grab tank items. Debatable whether you want to avoid Presence (to keep the stuns around) or prioritise it (to soak hits with a low value hero).
